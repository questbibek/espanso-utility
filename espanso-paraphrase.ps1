...
5. Output ONLY the paraphrased text - no explanation, no alternatives, no preamble, no labels, no quotes, no markdown"
        }
        @{ role = "user"; content = $originalText }
    )
    $reply = & "$PSScriptRoot\ai-call.ps1" -Messages $messages -Temperature 0.4 -MaxTokens 2000
    if ($reply) {
