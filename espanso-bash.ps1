...
if ($originalText -and $originalText.Length -gt 0) {
    ...
    $body = @{
        ...
        messages = @(
            @{
                role = "system"
                content = "You are a Git Bash expert on Windows. The user describes what they want to do. Return ONLY the exact bash command ready to run — no explanation, no markdown, no code blocks, no comments."
            }
            @{
                role = "user"
    ...
    try {
        ...
        $errorMsg = "Error: Unable to generate bash command"
